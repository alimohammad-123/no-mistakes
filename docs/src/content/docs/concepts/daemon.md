...
A historical run can fail in Document at the three-target replay boundary even after the exact candidate passed configured Test. When AXI reports that exact capacity error, an operator may request the guarded same-run transition with [`no-mistakes axi recover-final-head`](/no-mistakes/reference/cli/#no-mistakes-axi-recover-final-head).
...
The resumed executor starts at Document without rerunning or weakening the exact Test proof. The [Document step reference](/no-mistakes/reference/pipeline-steps/#document) owns the isolated boundary-assessment contract. A daemon shutdown during the resumed suffix keeps the recovery event, proof, run ID, and worktree for ordinary active-run recovery. If a process crash leaves Push custody active, startup first proves that no live executor owns it and compares the canonical remote ref with the durable recovery phase before clearing custody. Before revival, the daemon anchors the exact recovered head under a private immutable Git ref and records that ref in recovery provenance. The anchor survives supersession and worktree cleanup, and is retired only after another guarded ref proves equivalent reachability. Each external Push or PR mutation runs while a prepared Git ref transaction holds the exact source ref at the recovered head, so receive-side updates cannot cross the delivery boundary. Before revival, recovered Push also proves the remote supports atomic multi-ref updates. Its non-mutating capability probe skips local push hooks, but the real recovered Push preserves the repository's configured pre-push hook policy. It updates the target branch and a unique absent leased receipt ref in one remote transaction, retaining that receipt through PR and CI. Startup accepts only the exact target and receipt pair; target-only, receipt-only, rollback, collision, deletion, duplicate, malformed, or third-OID state is terminal ambiguity and can never rotate or republish. A newer gate push outside that ownership interval leaves both commits untouched, marks the recovered run superseded, and prevents delivery. Delivery recovery binds an exact remotely applied head without publishing again, retries only an unchanged earlier head with no receipt, and journals the PR's prior and intended content before mutation so restart can distinguish a safe retry from an already-applied update or ambiguous external drift. GitHub, GitLab, and Azure DevOps expose an explicit authoritative recovery-snapshot capability. The daemon requires that capability and validates repository, identity, open-unmerged lifecycle, source head and branch, base branch, title, and body before revival, then revalidates the target and receipt while holding delivery ownership immediately before PR mutation and after the authoritative post-mutation snapshot. CI repeats pair validation after every provider poll and before readiness or terminal decisions, including provider errors, merged-snapshot errors, and cancellation. Only confirmed durable ambiguity supersedes the original provider or cancellation cause; transient custody-check failures cannot mask it. A durable ambiguity entry is monotonic: a later restoration of the live pair cannot heal it, and ambiguity atomically clears and forbids readiness. Provider or CLI unavailability may skip ordinary CI, but it stops the exact recovered suffix. A merged PR may have a deleted source branch only when an authoritative merged snapshot proves the exact recovered head and the immutable receipt remains intact. Unsupported or incomplete providers are refused before Push.
