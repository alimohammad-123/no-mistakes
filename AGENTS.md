...
- The sole absent-policy exception is global `bootstrap.test`: it can authorize only Test after exact repository, frozen base, user-owned command, and submitted policy-byte digest checks. The complete authorization is write-once in `runs` before execution; recovery never uses mutable bootstrap values to replace it and refuses once base policy exists. The submitted policy is evidence only, never the command source. See `internal/daemon/bootstrap.go` and `internal/db/run.go`.
