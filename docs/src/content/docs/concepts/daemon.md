...
The resumed executor starts at Document without rerunning or weakening the exact Test proof. Document uses an isolated disposable clone at the boundary and can continue only when that assessment is a no-op. Any proposed change fails before a commit or source-ref advance. A daemon shutdown during the resumed suffix keeps the recovery event, proof, run ID, and worktree for ordinary active-run recovery. If a process crash leaves Push custody active, startup first proves that no live executor owns it and compares the canonical remote ref with the durable recovery phase before clearing custody. The recovery event's exact source ref and head are checked with expected-ref CAS after remote probes, again at executor admission, and immediately before Push or PR mutation. A newer gate push leaves both commits untouched, marks the recovered run superseded, and prevents delivery. Delivery recovery binds an exact remotely applied head without publishing again, retries only an unchanged earlier head, and journals the PR's prior and intended content before mutation so restart can distinguish a safe retry from an already-applied update or ambiguous external drift.
