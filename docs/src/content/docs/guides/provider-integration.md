...
Re-running `no-mistakes init` later preserves the stored fork URL unless you pass a new `--fork-url`. Pipeline-base policy is resolved independently and always validated and fetched from the parent, never the fork; see [`base_branch`](/no-mistakes/reference/repo-config/#base_branch) for its precedence.
