...
| Flag          | Type     | Default | Description                                                               |
| ------------- | -------- | ------- | ------------------------------------------------------------------------- |
| `--intent`    | `string` | (none)  | What the user set out to accomplish; required to start or recover a run   |
...
Claiming the bounded compatibility path for a legacy approval gate failed by graceful shutdown does require the same authoritative intent; the [daemon lifecycle reference](/no-mistakes/concepts/daemon/#interrupted-approval-compatibility) owns the exact eligibility and refusal rules.
...
If delivery is interrupted, recovery first refuses any live executor owner, then verifies the canonical remote ref against the durable Push phase before clearing stale custody. The recovery event records a private immutable Git ref anchoring the exact recovered head before revival or cleanup. That anchor survives supersession and is retired only after guarded equivalent-reachability proof. Each external Push or PR mutation holds a prepared Git transaction on the exact source ref, so a receive-side update cannot move it before or during delivery. Recovered Push additionally requires atomic multi-ref support and updates the target branch together with a unique leased remote receipt ref in one transaction. The capability probe is non-mutating and skips local push hooks; the actual transaction runs configured pre-push hooks and a rejection leaves both refs unchanged. Restart accepts only the exact target and receipt pair, retains the receipt through the delivery suffix, and treats missing, split, rolled-back, collided, or malformed pairs as permanent ambiguity. A newer gate push outside that ownership interval cancels the recovered run as superseded without moving either commit or mutating remote delivery. Recovery binds an exact remotely applied head without publishing again and retries only when the earlier remote head is unchanged and the receipt is absent. The existing PR update records its canonical identity, prior-content digest, intended content, and applied state so restart can finish an exact already-applied update without repeating it, retry only an unchanged prior state, and refuse partial or superseded content. GitHub, GitLab, and Azure DevOps declare an authoritative recovery-snapshot capability. Recovery requires it and validates the configured repository, PR identity, open and unmerged lifecycle, exact source head and branch, frozen base branch, title, and body before revival. While holding delivery ownership it revalidates the target and receipt immediately before PR mutation and after the authoritative post-mutation snapshot. CI revalidates after every provider poll and before readiness or terminal decisions, including provider errors, merged-snapshot errors, and cancellation. Only confirmed durable ambiguity supersedes the original provider or cancellation cause; transient custody-check failures cannot mask it. Once durable ambiguity is recorded, a restored live pair cannot heal it; readiness remains forbidden. Provider or CLI unavailability may skip ordinary CI, but it stops the exact recovered suffix. Source deletion after merge is accepted only when an authoritative merged snapshot proves the exact recovered head and the receipt remains intact. Unsupported or incomplete providers are refused before Push.
