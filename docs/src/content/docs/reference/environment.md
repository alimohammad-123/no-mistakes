...
Before a configured command or stable-candidate agent runs, the runtime requires the isolated pipeline repository's `HEAD` and canonical source ref to match the exact recorded candidate commit, including pipeline fix commits. A mismatch is refused before the child process starts. Pipeline-owned candidate changes advance the ref with a compare-and-swap from the prior recorded head; an already-advanced ref is accepted only for an idempotent durable-state retry, while a superseding receive-side move is never rewound. The same exact binding is rechecked at delivery and CI-readiness boundaries. This does not move the developer's worktree branch or contact a remote; the normal Push step remains the first remote update. The runtime removes any inherited `NO_MISTAKES_SOURCE_REF` and appends its frozen value last at child-process launch. Manually exporting this variable for a direct local invocation is not delivery provenance and cannot override the pipeline-owned value.
