...
- During first policy adoption only, the user-owned global [`bootstrap.test`](/no-mistakes/reference/global-config/#bootstraptest) binding can supply that baseline command when the freshly pinned base proves policy absent. The exact binding is frozen before execution, only Test is authorized, and base-owned policy takes permanent precedence once present.
